function handleLogin() {
    var e = [],
        o = window.swaggerUiAuth.authSchemes || window.swaggerUiAuth.securityDefinitions;
    if (o) {
        var i, a = o;
        for (i in a) {
            var n = a[i];
            if ("oauth2" === n.type && n.scopes) {
                ...
                if (Array.isArray(n.scopes)) {
                    ...
                    for (t in n.scopes) e.push({
                        scope: t,
                        description: n.scopes[t],
                        OAuthSchemeKey: i
                    })
            }
        }
    ...
    popupDialog.css({
        ...
        var e, o = window.swaggerUi.api.authSchemes,
            i = window.location,
            a = location.pathname.substring(0, location.pathname.lastIndexOf("/")),
            n = i.protocol + "//" + i.host + a + "/o2c.html",
            t = window.oAuthRedirectUrl || n,
            p = null,
            r = [],
        ...
        for (var u in o)
            if (o.hasOwnProperty(u) && s.indexOf(u) != -1) {
                var h = o[u].flow;
                if ("oauth2" !== o[u].type || !h || "implicit" !== h && "accessCode" !== h) {
                    ...
                    if (o[u].grantTypes) {
                        ...
                        for (var w in c)
                            if (c.hasOwnProperty(w) && "implicit" === w) {
                                var g = c[w];
                                g.loginEndpoint.url;
